-- Add Enschede as the fourth backend location while keeping Rotterdam as reference/background data.
-- The active patient-facing demo locations are managed by the website/orchestration layer.
...
begin
  ...
  if (select count(*) from public.locations where location_id in ('LOC-AMS','LOC-RTM','LOC-UTR','LOC-ENS'))<>4 then
  ...
  if exists (select 1 from public.locations l left join public.rooms r on r.location_id=l.location_id
      where l.location_id in ('LOC-AMS','LOC-RTM','LOC-UTR','LOC-ENS') group by l.location_id having count(r.room_id)<>5) then
  ...
  where l.location_id in ('LOC-AMS','LOC-RTM','LOC-UTR','LOC-ENS');
  ...
  if v_slots<>36*p_generated_days then raise exception using errcode='P0001',message='Unexpected slot count after reset'; end if;
